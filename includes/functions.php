 ...
 * @return mixed
 */
function bluefrog_addons_get_translated_object_id( $id, $type = 'page', $original = true, $language_code = null ) {
 ...
 * @return array
 */
function bluefrog_addons_get_terms_hierarchy( $taxonomy = 'category', $separator = '-' ) {
	...
	if ( $taxonomy->hierarchical ) {
		$terms = bluefrog_addons_sort_terms_hierarchy( $terms );
		$terms = bluefrog_addons_flatten_hierarchy_terms( $terms, $separator );
	}
 ...
 * @return array
 */
function bluefrog_addons_sort_terms_hierarchy( $terms, $parent_id = 0 ) {
	$hierarchy = array();

	foreach ( $terms as $term ) {
		if ( $term->parent == $parent_id ) {
			$term->children = bluefrog_addons_sort_terms_hierarchy( $terms, $term->term_id );
			$hierarchy[] = $term;
		}
	}
 ...
 * @return array
 */
function bluefrog_addons_flatten_hierarchy_terms( $terms, $separator = '&mdash;', $depth = 0 ) {
	$flatted = array();
	...
	foreach ( $terms as $term ) {
		...
		if ( ! empty( $children ) ) {
			$flatted = array_merge( $flatted, bluefrog_addons_flatten_hierarchy_terms( $children, $separator, ++$depth ) );
			$depth--;
		}
	}
